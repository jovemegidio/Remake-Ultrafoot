...
const CFG={
  ...
  ball:{
    ...
    spinDecay:0.36,      // 1/s — decaimento da rotação no ar
    e:0.48,              // restituição em gramado aparado
    mu:0.34,             // atrito tangencial no quique
    roll:0.43            // resistência ao rolamento (m/s²)
  },
  air:{ rho:1.225, wind:0.8, windDir:0.7, gust:.12 },
  speed:{ run:6.5, sprint:9.0, gk:5.6, accel:7.4, turn:6.2 },
  ...
  teams:{
    home:{ tag:opcoes.casa?.sigla||'CAS', kit:opcoes.casa?.corPrincipal||'#e0402a',
           trim:opcoes.casa?.corSecundaria||'#7d1a0e', shorts:opcoes.casa?.corSecundaria||'#f4f6f8',
           socks:opcoes.casa?.corPrincipal||'#e0402a', gk:'#39d17f', dir: 1,
           name:opcoes.casa?.nome||'MANDANTE' },
    away:{ tag:opcoes.fora?.sigla||'FOR', kit:opcoes.fora?.corPrincipal||'#eef2f6',
           trim:opcoes.fora?.corSecundaria||'#1b2a5e', shorts:opcoes.fora?.corSecundaria||'#e7ecf1',
           socks:opcoes.fora?.corPrincipal||'#eef2f6', gk:'#f2d032', dir:-1,
           name:opcoes.fora?.nome||'VISITANTE' },
...
const Gfx={
  ...
  init(){
    ...
    try{
      ...
      this.rtScene.samples=QUALITY==='high'?4:0;
  ...
  render(dt,camSpeed){
    ...
    if(!this.post){ renderer.setRenderTarget(null); renderer.setViewport(0,0,cw,ch); renderer.render(scene,camera); return; }
    this.t+=dt;
...
function initRenderer(){
  const tamanho = () => {
    const r = _palco.getBoundingClientRect()
    return { w:Math.max(320,Math.round(r.width||1280)), h:Math.max(240,Math.round(r.height||720)) }
  }
  ...
  const redimensionar=(entradas)=>{
    ...
    const atual=caixa
      ? {w:Math.max(320,Math.round(caixa.width)),h:Math.max(240,Math.round(caixa.height))}
      : tamanho()
    camera.aspect=atual.w/atual.h; camera.updateProjectionMatrix();
    renderer.setSize(atual.w,atual.h,false);
    Gfx.resize();
  }
  _on(window,'resize',redimensionar);
  if(typeof ResizeObserver!=='undefined'){
    _resizeObserver=new ResizeObserver(redimensionar)
    _resizeObserver.observe(_palco)
  }
}
...
function kick(from,dirV,power,loft,side,top){
  ...
  d.normalize();
  if(from){
    from.facePrev=from.face
    from.face=Math.atan2(d.x,d.z)
  }
  ...
  if(from){ from.kickT=.42; from.pose='kick'; from.poseT=.42; }
  Audio2.kick(power);
}
...
/* ----------------------------------------------------------------------------
   LEI DO QUIQUE, isolada.
   Vive fora de `ballBounce` porque os PREVISORES precisam da mesma lei. Eles
   usavam um atalho (`vx*=.86`, sem atrito nem rotação): a bola que quicava
   antes da linha chegava num z diferente do previsto, e o goleiro mergulhava
   para o lado errado por erro de modelo, não por decisão dele. Um chute com
   efeito, que é justamente quando a leitura importa, era o pior caso.
   Recebe vetores soltos ({x,y,z}) para servir aos dois mundos.
   Devolve `false` quando o quique morreu — dali em diante a bola rola.
   ---------------------------------------------------------------------------- */
function quique(v,w){
  const C=CFG.ball, r=C.r;
  const vy=-v.y;
  if(vy<=.55){ v.y=0; return false; }
  v.y=vy*C.e;
  // velocidade do ponto de contato: v + ω × (0,-r,0)
  const cx=v.x+w.z*r, cz=v.z-w.x*r;
  const cl=Math.hypot(cx,cz);
  if(cl>1e-4){
    const jn=(1+C.e)*vy;
    // cl/3.5 = 2/7·cl, o impulso que leva a esfera maciça ao rolamento puro:
    // acima disso o pé de atrito seria maior que o necessário e a bola voltaria.
...
function ballIntegrate(h){
  ...
  /* ------------------------------------------------------------------------
     REDE — colisão de verdade, e não decoração.

     A contenção morava em `Rules.step`, dentro do ramo `phase==='play'`. Só que
     no gol a fase vira 'goal' e o `step` retorna ANTES: a bola seguia reta e
     atravessava a rede justamente no lance em que todo mundo está olhando.
     Aqui ela é parte do integrador, então vale em qualquer fase.

     A rede não quica: ela absorve. Coeficiente baixo (0,18) e perda tangencial
     forte — a bola morre dentro do gol, como no futebol.

     A porta de entrada é EXATAMENTE a boca do gol: entre os postes e abaixo do
     travessão. Com uma folga, uma bola que passa raspando por fora do pau era
     agarrada pelo pano lateral e puxada para dentro — o motor sequestrava a
     bola justamente no lance em que ela saiu.
     ------------------------------------------------------------------------ */
  if(Math.abs(B.pos.x)>HALF_L-r && Math.abs(B.pos.z)<CFG.goal.w/2
     && B.pos.y<CFG.goal.h){
    const s=Math.sign(B.pos.x)||1, eN=.18, atrito=.55;
    const fundo=HALF_L+CFG.goal.depth, meia=CFG.goal.w/2;
    // a velocidade de CHEGADA é medida antes de refletir: depois dela sobra 18%
    let bateu=0;
    if(Math.abs(B.pos.x)>fundo-r){                       // pano do fundo
      B.pos.x=s*(fundo-r);
      if(v.x*s>0){ bateu=Math.abs(v.x); v.x=-v.x*eN; v.y*=atrito; v.z*=atrito; }
    }
    if(Math.abs(B.pos.x)>HALF_L+.05){                    // já dentro do gol
      if(Math.abs(B.pos.z)>meia-r){                      // panos laterais
        const sz=Math.sign(B.pos.z)||1;
        B.pos.z=sz*(meia-r);
        if(v.z*sz>0){ bateu=Math.max(bateu,Math.abs(v.z)); v.z=-v.z*eN; v.x*=atrito; }
      }
      if(B.pos.y>CFG.goal.h-r){                          // pano de cima
        B.pos.y=CFG.goal.h-r;
        if(v.y>0){ bateu=Math.max(bateu,Math.abs(v.y)); v.y=-v.y*eN; v.x*=atrito; v.z*=atrito; }
      }
    }
    if(bateu>1.5){
      // o `t` da rede também serve de cooldown do som: a bola morrendo dentro
      // do gol toca vários panos seguidos, e isso vira metralhadora no áudio
      const net=World.nets[s<0?0:1];
      const soar=!net||net.t>.28;
      hitNet(s,B.pos.x,B.pos.y,B.pos.z,bateu);
      w.multiplyScalar(.25);                             // o pano come a rotação
      if(soar&&Audio2.net) Audio2.net();
    }
  }
...
/* passo da bola: subdivide para que nenhum subpasso ande mais de 5 cm.
   ...
   O teto era 12 subpassos, e ele quebrava a promessa acima de 36 m/s: um chute
   de 40 m/s andava 5,5 cm por subpasso, mais que o raio de colisão da trave
   (6 cm + raio da bola), e a bola atravessava o pau em vez de bater. Com 24 a
   regra dos 5 cm vale até 72 m/s, acima de qualquer chute humano. Custa apenas
   nos poucos quadros em que a bola está muito rápida. */
function ballStep(dt){
  ...
  const n=clamp(Math.ceil(sp*dt/.05),1,24);
  B.subs=n;
  const h=dt/n;
  for(let i=0;i<n;i++) ballIntegrate(h);
}

/* Vetores de rascunho dos previsores. Ficam no módulo porque `predictAtPlane`
   roda uma vez por goleiro por quadro: alocar aqui seria lixo por quadro. */
const _pv={x:0,y:0,z:0}, _pw={x:0,y:0,z:0};
...
function predictTrajectory(out,dur){
  ...
  for(let t=0;t<dur;t+=h){
    ...
    if(py<=r){
      py=r;
      _pv.x=vx; _pv.y=vy; _pv.z=vz; _pw.x=wx; _pw.y=wy; _pw.z=wz;
      const seguiu=quique(_pv,_pw);
      vx=_pv.x; vy=_pv.y; vz=_pv.z; wx=_pw.x; wy=_pw.y; wz=_pw.z;
      if(!seguiu) break;                       // quique morto: daqui é rolagem
    }
...
function predictAtPlane(gx){
  ...
  for(let t=0;t<2.2;t+=h){
    ...
    if(py<=r){
      ...
      vx=_pv.x; vy=_pv.y; vz=_pv.z; wx=_pw.x; wy=_pw.y; wz=_pw.z;
      if(!seguiu){
        // rolando: o que freia é o rolamento, não o arrasto. Sem isto a bola
        // rasteira chegava ao plano do gol mais cedo do que chega de verdade.
        const hv=Math.hypot(vx,vz), rr=C.roll*h;
        if(hv>rr){ vx-=vx/hv*rr; vz-=vz/hv*rr; } else return null;  // parou antes
      }
    }
...
class Player{
  ...
  moveTo(t,dt,urgency){
    ...
    let wx=0,wz=0;
    if(d>.1){
      // Velocidade de chegada pela distância de frenagem v²=2ad. O modelo
      // anterior sempre pedia ao menos 1,1 m/s e fazia o atleta oscilar ao
      // redor do ponto em vez de plantar o pé e parar.
      const freio=CFG.speed.accel*1.18
      const want=Math.min(this.maxSpeed,Math.sqrt(2*freio*d))*(urgency||1);
  ...
  render(a){
    ...
    if(this.pose==='dive'){
    ...
    } else if(this.pose==='control'){
      const t=1-clamp(this.poseT/.34,0,1)
      const amort=Math.sin(t*Math.PI)
      L[1].rotation.x=-.48*amort
      L[1].userData.knee.rotation.x=.62*amort
      L[1].userData.boot.rotation.x=.28*amort
      A[0].rotation.x=-.18*amort; A[1].rotation.x=.18*amort
      rotX=.08*amort; y=-.025*amort
      ...
      A[0].rotation.x=-sw*amp*.80; A[1].rotation.x=sw*amp*.80;
      ...
      if(sp<.2){
        // Repouso vivo, sem marcha no lugar: só respiração e transferência
        // mínima de peso.
        y=Math.sin(this.phase*.72)*.007
        A[0].rotation.x=Math.sin(this.phase*.72)*.018
        A[1].rotation.x=-A[0].rotation.x
      }
      ...
      if(this.kickT>0){
        const u=clamp(1-this.kickT/.42,0,1);
...
/* Com que velocidade o passe rasteiro deve CHEGAR ao companheiro.
   Era `3,6 + L*0,04` limitado a 5,6 m/s: um passe de 20 m saía a 9,6 m/s e
   chegava a 4,4, ou seja, quase 3 segundos rolando para atravessar 20 metros.
   Ninguém joga assim — o passe morria no caminho e a defesa sempre chegava
   antes. A escala nova põe o mesmo passe em ~1,9 s, que é o tempo de bola de
   um jogo de verdade. O teto de 8,5 m/s existe porque a chegada entra numa
   exponencial: pedir 11 m/s a 40 m exigiria sair a 32 m/s, um foguete. */
const passArrive=L=>clamp(4.2+L*.16,4.5,8.5);

/* Tempo de voo de um passe alto: é o MESMO t que o chute balístico abaixo
   escolhe, e não uma média inventada. */
const passLoftT=L=>clamp(L/15,.55,1.9);

/* Tempo que a bola leva para desacelerar de v0 até va rolando.

   Tem solução fechada: com dv/dt = -(k·v² + r₀),
     t = [atan(v·√(k/r₀))] / √(k·r₀), avaliado entre va e v0.

   Mas `k` não é constante — cai pela metade na crise do arrasto, em torno de
   13,5 m/s. Aplicar a fórmula de uma vez erra 37% num passe forte (28 m/s dá
   6,7 s onde a integração numérica dá 4,9). Em quatro trechos, cada um com o
   `k` da sua faixa de velocidade, o erro cai para menos de 1% — a 4 arco-
   tangentes, que é barato para algo que roda uma vez por passe.

   Por que não estimar por uma média entre saída e chegada: a queda é
   exponencial, não linear, e o erro vira passe atrasado — que é passe perdido. */
function passFlightT(v0,va){
  const r0=CFG.ball.roll;
  let t=0, a=v0;
  for(let i=1;i<=4;i++){
    const b=v0+(va-v0)*i/4;
    const k=Aero.k((a+b)*.5);
    const c=Math.sqrt(k/r0), s=Math.sqrt(k*r0);
    t+=(Math.atan(a*c)-Math.atan(b*c))/s;
    a=b;
  }
  return clamp(t,.15,3);
}
...
function passTo(p,m,lofted){
  ...
  //
  // Mirar `vel * 0.46` assumia que a bola chega em meio segundo. Em 20 m ela
  // leva bem mais, e o companheiro ja andou vários metros — o passe ia para
  // onde ele esteve. Estimamos o voo e miramos o ponto futuro.
  //
  // O tempo agora sai do MESMO modelo que vai chutar: `passPower` resolve a
  // saida, `passArrive` da a chegada e `passFlightT` integra o tempo entre as
  // duas. Antes a mira usava uma constante que discordava do chute — o passe
  // era liderado com um tempo de voo que a fisica nunca produzia.
  //
  ...
  for(let i=0;i<2;i++){
    ...
    let tVoo;
    if(lofted) tVoo = passLoftT(dParcial);
    else {
      const chega = passArrive(dParcial);
      tVoo = passFlightT(passPower(dParcial,chega), chega);
    }
  ...
  if(lofted){                    // balístico: escolhe o tempo de voo e resolve vy/vx
    ...
    power=passPower(L,passArrive(L)); loft=.32;
  }
...
function takeBall(p){
  ...
  if(inc>3.2){
    p.pose='control'; p.poseT=.34
    // O primeiro toque absorve parte do momento antes de prender a bola ao
    // drible. A animação mostra a perna recuando, não um ímã instantâneo.
    p.vel.x+=b.vel.x*.035; p.vel.z+=b.vel.z*.035
  }
...
const Rules={
  ...
  goal(team,power){
    ...
    /* A rede NÃO é sacudida daqui.
       Era: `hitNet(side, Ball.pos.z, Ball.pos.y, power)` — a onda saía no ponto
       onde a bola estava no instante em que o gol foi CONTABILIZADO. Num gol
       encenado isso é dez metros antes de ela chegar: a rede balançava sozinha,
       com a bola ainda no ar, e depois a bola entrava numa rede parada.
       Agora quem sacode é a colisão de verdade, no ponto e no instante do
       toque. O som acompanha, pelo mesmo motivo. */
    Audio2.cheer(1); Audio2.whistle(false);
  ...
  step(dt){
    ...
    /* A contenção da rede saiu daqui: virou colisão no integrador (`ballStep`).
       Neste ramo ela só valia durante `phase==='play'`, e no gol — a única hora
       em que a bola de fato entra na rede — a fase já é 'goal' e o `step`
       retorna antes de chegar nesta linha. */
...
/* ============================================================================
   RADAR — a planta do campo em perspectiva.

   Ele não é um enfeite: é a única visão que mostra os 22 ao mesmo tempo, e a
   câmera de TV esconde metade do campo por definição. Por isso ele carrega o
   que a câmera não dá — onde estão todos, e o que a BOLA está fazendo.

   O que veio da física, e não de desenho:
     · altura — a bola sobe na tela e a sombra dela abre no chão, exatamente
       como no campo. Um cruzamento e um passe rasteiro deixaram de ter o mesmo
       desenho, que era o defeito mais gritante da versão anterior.
     · rastro — as posições reais dos últimos quadros. A curva de um chute com
       efeito aparece curva, porque é curva.
     · queda prevista — integra a MESMA física (`predictTrajectory`) para dizer
       onde a bola vai cair. Não é palpite: é o integrador rodando à frente.
     · direção — cada jogador tem o rumo do corpo, que é o que deixa ler quem
       está de frente para o gol e quem está voltando.
   ============================================================================ */
let rc=null;
const _rastro=[];                      // {x,z,y} dos últimos quadros da bola
const _predBuf=new Float32Array(120);  // buffer da previsão (40 pontos xyz)
...
function drawRadar(){
  ...
  const top=13, hgt=H-27, cx=W/2, hw=W/2-14;
  /* Projeção: o fundo do campo é mais estreito que a frente. `sc` é o
     encurtamento lateral, e é ele que dá a sensação de estar olhando o campo
     de uma cabine, e não de um satélite. */
  const prof=z=>(clamp(z,-HALF_W-3,HALF_W+3)+HALF_W)/(2*HALF_W);
  const P=(x,z)=>{
    const d=prof(z), sc=lerp(.56,1,d);
    return [cx+clamp(x,-HALF_L-4,HALF_L+4)/HALF_L*hw*sc, top+d*hgt];
  };
  const linha=(pts,fechar)=>{
    g.beginPath();
    for(let i=0;i<pts.length;i++){
      const q=P(pts[i][0],pts[i][1]);
      i?g.lineTo(q[0],q[1]):g.moveTo(q[0],q[1]);
    }
    if(fechar) g.closePath();
    g.stroke();
  };
  const quad=(x0,x1,z0,z1,cor)=>{
  ...
  const arco=(cxm,czm,r,a0,a1,passos)=>{
    const pts=[];
    for(let i=0;i<=passos;i++){
      const a=a0+(a1-a0)*i/passos;
      pts.push([cxm+Math.cos(a)*r, czm+Math.sin(a)*r]);
    }
    linha(pts);
  };

  // ── grama: faixas de corte, que é o que faz o desenho parecer um campo ────
  const FAIXAS=10, passo=CFG.pitch.L/FAIXAS;
  for(let i=0;i<FAIXAS;i++){
    const x0=-HALF_L+i*passo;
    quad(x0,x0+passo,-HALF_W,HALF_W, i%2 ? 'rgba(20,62,41,.88)' : 'rgba(15,50,34,.88)');
  }
  // sombra do estádio no fundo: escurece a faixa distante e afunda o campo
  const sombra=g.createLinearGradient(0,top,0,top+hgt);
  sombra.addColorStop(0,'rgba(4,10,8,.55)');
  sombra.addColorStop(.45,'rgba(4,10,8,0)');
  quad(-HALF_L,HALF_L,-HALF_W,HALF_W,sombra);

  // ── marcação ──────────────────────────────────────────────────────────────
  g.strokeStyle='rgba(232,242,250,.42)'; g.lineWidth=1.4;
  linha([[-HALF_L,-HALF_W],[HALF_L,-HALF_W],[HALF_L,HALF_W],[-HALF_L,HALF_W]],true);
  linha([[0,-HALF_W],[0,HALF_W]]);
  arco(0,0,9.15,0,6.2832,30);                                   // círculo central
  for(const s of [-1,1]){
    linha([[s*HALF_L,-20.16],[s*(HALF_L-16.5),-20.16],
           [s*(HALF_L-16.5),20.16],[s*HALF_L,20.16]]);           // grande área
    linha([[s*HALF_L,-9.16],[s*(HALF_L-5.5),-9.16],
           [s*(HALF_L-5.5),9.16],[s*HALF_L,9.16]]);              // pequena área
    // meia-lua: só o trecho que sai da grande área
    const pm=s*(HALF_L-11), lim=s*(HALF_L-16.5);
    const pts=[];
    for(let i=0;i<=24;i++){
      const a=i/24*6.2832, x=pm+Math.cos(a)*9.15, z=Math.sin(a)*9.15;
      if(s>0 ? x<lim : x>lim) pts.push([x,z]); else if(pts.length){ linha(pts); pts.length=0; }
    }
    if(pts.length) linha(pts);
    /* arcos de escanteio: um QUARTO de círculo, virado para dentro. O círculo
       inteiro fazia quatro bolhas nos cantos, que não existem em campo nenhum. */
    for(const t of [-1,1]){
      const a0=s>0?Math.PI:0;
      arco(s*HALF_L,t*HALF_W,1,a0,a0+(s*t>0?Math.PI/2:-Math.PI/2),6);
    }
  }
  // marcas de cal: centro e penalti
  g.fillStyle='rgba(232,242,250,.5)';
  for(const m of [[0,0],[HALF_L-11,0],[-(HALF_L-11),0]]){
    const q=P(m[0],m[1]);
    g.beginPath(); g.arc(q[0],q[1],1.3,0,6.2832); g.fill();
  }
  /* Gols na cor de quem DEFENDE aquele lado. `dir` é a direção de ATAQUE:
     o gol em +dir do mandante é o gol do visitante. Trocar isso pinta cada
     equipe atrás da meta errada, e o radar passa a mentir de que lado se joga. */
  for(const s of [-1,1]){
    const dono=CFG.teams.home.dir===s?'away':'home';
    g.strokeStyle=CFG.teams[dono].kit; g.globalAlpha=.75; g.lineWidth=1.6;
    linha([[s*HALF_L,-3.66],[s*(HALF_L+2),-3.66],[s*(HALF_L+2),3.66],[s*HALF_L,3.66]]);
    g.globalAlpha=1;
  }

  // ── rastro da bola: posições REAIS, não interpolação de desenho ───────────
  const bx=Ball.pos.x, bz=Ball.pos.z, by=Ball.pos.y;
  const ult=_rastro[_rastro.length-1];
  // um salto grande é reposição (falta, escanteio, tiro de meta): corta o rastro
  if(ult && (Math.abs(ult.x-bx)>6 || Math.abs(ult.z-bz)>6)) _rastro.length=0;
  _rastro.push({x:bx,z:bz,y:by});
  if(_rastro.length>34) _rastro.shift();

  if(_rastro.length>2){
    for(let i=1;i<_rastro.length;i++){
      const a=_rastro[i-1], b=_rastro[i];
      const t=i/_rastro.length;
      const qa=P(a.x,a.z), qb=P(b.x,b.z);
      g.strokeStyle='rgba(255,255,255,'+(t*t*.5).toFixed(3)+')';
      g.lineWidth=lerp(.6,2.2,t);
      g.beginPath();
      g.moveTo(qa[0],qa[1]-a.y*3.1); g.lineTo(qb[0],qb[1]-b.y*3.1);
      g.stroke();
    }
  }

  // ── onde a bola vai cair, pela física e não por palpite ───────────────────
  if(!Ball.owner && by>.55 && Ball.vel.lengthSq()>4){
    const n=predictTrajectory(_predBuf,2.4);
    for(let i=n-1;i>0;i--){
      if(_predBuf[i*3+1]<=CFG.ball.r*1.6){
        const q=P(_predBuf[i*3],_predBuf[i*3+2]);
        g.strokeStyle='rgba(255,255,255,.30)'; g.lineWidth=1;
        g.beginPath(); g.arc(q[0],q[1],3.4,0,6.2832); g.stroke();
        g.beginPath(); g.moveTo(q[0]-2,q[1]); g.lineTo(q[0]+2,q[1]);
        g.moveTo(q[0],q[1]-2); g.lineTo(q[0],q[1]+2); g.stroke();
        break;
      }
    }
  }

  // ── jogadores, do fundo para a frente ─────────────────────────────────────
  const lista=players.map(p=>({p,d:prof(p.pos.z),q:P(p.pos.x,p.pos.z)}))
    .sort((a,b)=>a.d-b.d);
  for(const k of lista){
    const p=k.p, r=lerp(3.1,5.4,k.d), x=k.q[0], y=k.q[1];
    // sombra de contato: cola o jogador no gramado
    g.fillStyle='rgba(0,0,0,.38)';
    g.beginPath(); g.ellipse(x,y+r*.42,r*.95,r*.42,0,0,6.2832); g.fill();

    const time=CFG.teams[p.team];
    g.fillStyle=p.gk?time.gk:time.kit;
    g.strokeStyle='rgba(3,8,6,.75)'; g.lineWidth=1;
    g.beginPath(); g.arc(x,y,r,0,6.2832); g.fill(); g.stroke();

    // rumo do corpo: no 3D `face` tem 0 apontando para +z
    const fx=Math.sin(p.face), fz=Math.cos(p.face);
    g.strokeStyle='rgba(255,255,255,.85)'; g.lineWidth=1.4;
    g.beginPath(); g.moveTo(x,y);
    g.lineTo(x+fx*r*1.55, y+fz*r*1.05);   // z encurtado, como o resto da planta
    g.stroke();

    if(p===Ball.owner){
      g.strokeStyle='#ffd34d'; g.lineWidth=1.6;
      g.beginPath(); g.arc(x,y,r*1.9,0,6.2832); g.stroke();
    }
  }

  // ── bola: sombra no chão + altura real na tela ────────────────────────────
  const q=P(bx,bz);
  const alt=Math.max(0,by-CFG.ball.r), sub=alt*3.1;
  g.fillStyle='rgba(0,0,0,'+(0.42/(1+alt*.5)).toFixed(3)+')';
  g.beginPath();
  g.ellipse(q[0],q[1],3+alt*.55,1.5+alt*.28,0,0,6.2832);
  g.fill();
  g.fillStyle='#fff';
  g.shadowColor='rgba(255,255,255,.9)'; g.shadowBlur=8;
  g.beginPath(); g.arc(q[0],q[1]-sub,lerp(2.8,4.1,prof(bz)),0,6.2832); g.fill();
  g.shadowBlur=0;
}
...
function simulate(dt){
  Aero.step(dt);
  // Fora de uma cena narrada o campo segura a posição. Assim o 3D não cria
  // uma segunda partida entre dois eventos do match-engine.
  if(_narrativaRestante<=0){
    for(const p of players){
      const f=Math.pow(.78,dt*60); p.vel.x*=f; p.vel.z*=f; p.step(dt)
    }
    Ball.vel.multiplyScalar(Math.pow(.72,dt*60))
    if(Ball.vel.lengthSq()<.002) Ball.vel.set(0,0,0)
    ballStep(dt); Officials.step(dt); return
  }
  _narrativaRestante=Math.max(0,_narrativaRestante-dt)
  Rules.step(dt);
...
function runBuild(done){
   ...
   *
   * Pode ser chamado antes ou durante a partida. Em campo, troca os pontos de
   * referência e os atletas chegam à nova estrutura correndo, sem teleporte.
   *
  ...
  function definirFormacao(slots){
    ...
    if(players.length === 22){
      for(const team of ["home", "away"]){
        const elenco = teams[team] || []
        elenco.forEach((jogador, indice) => {
          if(convertida[indice]) jogador.def = convertida[indice]
        })
      }
    }
    return true
  ...
  /** O goleiro de um lado. `teams[x][0]` era o atalho usado antes; e frágil. */
  function _goleiro(lado){
    const t = teams[lado]
    return t ? (t.find(p => p.gk) || null) : null
  }

  /** Volta a cena para o jogo corrido. Nao mexe em partida encerrada. */
  function _emJogo(){
    if(Match.phase !== 'end'){ Match.phase = 'play'; Match.wait = 0; Match.setType = null }
  }

  /**
   * Poe a bola no pe de um jogador de `lado`, a `dGol` metros do gol
   * adversario, e devolve quem vai finalizar.
   *
   * Existe porque os quatro eventos de finalizacao (chute, no alvo, para fora,
   * na trave) so diferem na MIRA — o resto do lance e o mesmo, e duplicar isso
   * quatro vezes era o caminho curto para quatro comportamentos diferentes.
   */
  function _armarFinalizador(lado, dGol){
    const d = CFG.teams[lado].dir
    const px = d * (HALF_L - dGol)
    const pz = rnd(-HALF_W * 0.42, HALF_W * 0.42)
    const atacante = _maisPertoDe(lado, px, pz)
    if(!atacante) return null
    atacante.pos.set(px, 0, pz); atacante.prev.copy(atacante.pos)
    atacante.vel.set(0, 0, 0)
    atacante.face = d > 0 ? Math.PI / 2 : -Math.PI / 2
    Ball.pos.set(px + d * 0.4, CFG.ball.r, pz); Ball.prev.copy(Ball.pos)
    Ball.vel.set(0, 0, 0); Ball.spin.set(0, 0, 0)
    Ball.owner = null; Ball.lastTouch = atacante
    Ball.cool = 0; Ball.shotCool = 0; Ball.offside = null; Ball.shotBy = null
    Ball.grace = 0.12                       // o suficiente para a bola sair do pe
    Match.possession = lado
    _emJogo()
    return atacante
  }

  /**
   * Finaliza ao gol a partir de um lance ja armado.
   *
   * A altura de chegada e RESOLVIDA, nao chutada: dada a potencia e a distancia,
   * o tempo de voo sai de t = d/v e o `loft` de vy = (y-r)/t + g·t/2. Antes o
   * loft era um `rnd(1.2, 3.3)` fixo — de 30 m a bola quicava duas vezes antes
   * do gol, e de 8 m saia por cima do travessao com a mesma conta.
   *
   * `mira` decide o DESFECHO FISICO, nunca o placar: encenar nao inventa gol.
   * Quem marca e o evento `goal`, que vem do match-engine. Por isso nenhuma
   * mira daqui aponta para um canto livre — sempre para o alcance do goleiro,
   * para o pau ou para fora.
   */
  function _finalizar(atirador, mira){
    const d = atirador.dir
    const gx = d * HALF_L
    const gk = _goleiro(atirador.team === 'home' ? 'away' : 'home')
    const meia = CFG.goal.w / 2                       // 3,66 m

    let aimZ, yAlvo, curva
    if(mira === 'fora'){
      aimZ = (Math.random() < .5 ? -1 : 1) * rnd(meia + .8, meia + 3.6)
      yAlvo = rnd(.6, 2.8); curva = rnd(-16, 16)
    } else if(mira === 'trave'){
      // 4 cm por FORA do pau: o desvio joga a bola para longe do gol, e nao
      // para dentro. Um rebote que entra mexeria num placar que nao e nosso.
      aimZ = (Math.random() < .5 ? -1 : 1) * (meia + .04)
      yAlvo = rnd(.4, 1.9); curva = 0
    } else {
      // no alvo, dentro do alcance do goleiro (1,52 m em pe, 2,35 no voo)
      const z0 = gk ? gk.pos.z : 0
      aimZ = clamp(z0 + (Math.random() < .5 ? -1 : 1) * rnd(1, 1.7), -meia + .4, meia - .4)
      yAlvo = rnd(.25, 1.5); curva = rnd(-9, 9)
    }

    const dz = aimZ - Ball.pos.z, dx = gx - Ball.pos.x
    const dGol = Math.hypot(dx, dz)
    const pot = clamp(dGol * .92 + rnd(7, 12), 17, 31)
    const tVoo = Math.max(.12, dGol / pot)            // o arrasto tira ~10%, e ok
    const loft = clamp((yAlvo - CFG.ball.r) / tVoo + 4.905 * tVoo, 0, 9.5)
    kick(atirador, new THREE.Vector3(dx, 0, dz), pot, loft, curva, rnd(-10, 4))
    Stats.shot(atirador.team)
    Audio2.cheer(.4)

    // CAMERA: baixa, atras do chutador — o plano que a TV usa no chute.
    Director.cena = { ang: Math.atan2(Ball.pos.z, Ball.pos.x) + Math.PI, raio: 19,
                      alt: 2.8, olha: 1.2, giro: .45, fecha: false, dur: 1.2, rate: 4.2 }
    Director.cenaT = 1.2
    return dGol
  }
  ...
  function encenar(evento){
    ...
    // Janela curta em que física, IA posicional e arbitragem podem executar o
    // lance descrito. Quando fecha, tudo repousa até a próxima linha narrada.
    const duracao = evento.tipo === "goal" ? 4.8
      : evento.tipo === "corner" || evento.tipo === "free_kick" ? 4.0
        : evento.tipo === "kickoff" ? 3.0 : 3.2
    _narrativaRestante = Math.max(_narrativaRestante, duracao)
    ...
    switch(evento.tipo){
      ...
      // ESCANTEIO — o mesmo caso serve para os dois times: `lado` e quem
      // COBRA, e `dir` leva a bandeirinha para o lado certo do campo. O
      // `setPiece` ja povoa a area com o ataque, marca homem a homem e poe o
      // goleiro adversario na linha; o `deliver` cobra em cruzamento alto.
      case "corner": {
        const d = CFG.teams[lado].dir
      ...
      // TIRO DE META — sai da PROPRIA linha de fundo, que fica em -dir.
      // Trocar o sinal aqui poe o goleiro cobrando dentro do gol adversario.
      case "goal_kick": {
        const d = CFG.teams[lado].dir
        Rules.setPiece(lado, -d * (HALF_L - 5.2), rnd(-8, 8), "TIRO DE META")
        return true
      }

      case "throw_in": {
        const d = CFG.teams[lado].dir
        Rules.setPiece(lado, d * rnd(-HALF_L * .5, HALF_L * .6),
                       (Math.random() < .5 ? -1 : 1) * HALF_W, "LATERAL")
        return true
      }

      // PASSE — quem passa e para quem, so isso. O passe em si e o `passTo` da
      // IA: mesma antecipacao do movimento do companheiro, mesmo erro angular
      // por qualidade/cansaco. Reimplementar aqui seria criar um segundo passe
      // que envelheceria separado do primeiro.
      case "pass": {
        const linha = (teams[lado] || []).filter(p => !p.gk)
        if(linha.length < 2) return false
        const de = pick(linha)
        const frente = linha.filter(m => m !== de && (m.pos.x - de.pos.x) * de.dir > 2)
        const para = frente.length ? pick(frente)
          : linha.filter(m => m !== de).sort((a, b) =>
              dist2(a.pos, de.pos) - dist2(b.pos, de.pos))[0]
        if(!para) return false

        Ball.pos.set(de.pos.x + Math.sin(de.face) * .4, CFG.ball.r,
                     de.pos.z + Math.cos(de.face) * .4)
        Ball.prev.copy(Ball.pos)
        Ball.vel.set(0, 0, 0); Ball.spin.set(0, 0, 0)
        Ball.owner = null; Ball.lastTouch = de; Ball.cool = 0; Ball.offside = null
        Match.possession = lado
        _emJogo()
        passTo(de, para, dist(de.pos, para.pos) > 26)   // longe demais: por cima
        return true
      }

      // CHUTE — finalizacao que NAO e gol (o gol tem evento proprio). Metade
      // vai no alvo para o goleiro pegar, metade passa perto. Quem decide qual
      // e o sorteio; quem decide o placar continua sendo o match-engine.
      case "shot": {
        const a = _armarFinalizador(lado, rnd(14, 26))
        if(!a) return false
        _finalizar(a, Math.random() < .55 ? "gol" : "fora")
        return true
      }

      case "shot_on_target": {
        const a = _armarFinalizador(lado, rnd(11, 22))
        if(!a) return false
        _finalizar(a, "gol")
        return true
      }
      ...
      case "miss": {
        const a = _armarFinalizador(lado, rnd(16, 30))
        if(!a) return false
        _finalizar(a, "fora")
        // CONSEQUENCIA: quem perdeu leva as maos a cabeca; a torcida geme.
        _cena(.55, () => {
          a.pose = 'maos_cabeca'; a.poseT = 2.6
          _reacaoDoTime(lado, 'maos_quadril', 2.2, .35)
        })
        return true
      }

      case "post": {
        const a = _armarFinalizador(lado, rnd(12, 24))
        if(!a) return false
        _finalizar(a, "trave")
        _cena(.7, () => { a.pose = 'maos_cabeca'; a.poseT = 2.8 })
        return true
      }

      // DEFESA — o chute e de verdade e a defesa tambem: o goleiro le a bola
      // pelo mesmo `predictAtPlane` de sempre. O que a encenacao faz e (1)
      // mirar dentro do alcance dele e (2) zerar a latencia de reacao, que e o
      // que separa uma defesa de um gol. Sem (2) o goleiro perderia os ~0,2 s
      // de leitura que ele gasta quando o chute o pega de surpresa.
      case "save": {
        const a = _armarFinalizador(lado, rnd(12, 22))
        if(!a) return false
        const gk = _goleiro(adv)
        // O goleiro vai para a linha ANTES do chute. Sem isto ele pode estar
        // fora de posicao por causa da cena anterior (um gol o deixa com as
        // maos na cabeca no meio da area) e a "defesa" vira gol — o 3D mexendo
        // num placar que nao e dele.
        if(gk){
          const d = CFG.teams[lado].dir
          gk.pos.set(d * (HALF_L - 1.1), 0, clamp(gk.pos.z, -1.2, 1.2))
          gk.prev.copy(gk.pos); gk.vel.set(0, 0, 0)
          gk.pose = 'run'; gk.poseT = 0
          gk.react = 0; gk.reactFor = a
        }
        _finalizar(a, "gol")
        _cena(.85, () => {
          if(gk){ gk.pose = 'celebrate'; gk.poseT = 1.4 }
          _reacaoDoTime(lado, 'maos_cabeca', 2, .3)
        })
        return true
      }

      // CONTRA-ATAQUE — nao e um lance, e uma TRANSICAO. Abre a janela que a
      // IA ja usa (`Match.counter`, que faz o passe vertical valer mais no
      // criterio de decisao) e desenha o campo que a define: quem perdeu a bola
      // ficou adiantado, quem a tem corre com espaco.
      case "counter_attack": {
        const d = CFG.teams[lado].dir
        const bx = d * rnd(-8, 4), bz = rnd(-HALF_W * .5, HALF_W * .5)
        const portador = _maisPertoDe(lado, bx, bz)
        if(!portador) return false

        Match.counter = { team: lado, t: 4.5 }
        // (nao adianta ligar `sprint` aqui: o `brain` zera o flag todo quadro
        //  e decide de novo pela stamina — quem corre e a IA, nao a cena)
        portador.pos.set(bx, 0, bz); portador.prev.copy(portador.pos)
        portador.vel.set(0, 0, 0)
        Ball.pos.set(bx + d * .4, CFG.ball.r, bz); Ball.prev.copy(Ball.pos)
        Ball.vel.set(0, 0, 0); Ball.spin.set(0, 0, 0)
        Ball.owner = portador; Ball.lastTouch = portador
        Ball.cool = .2; Ball.offside = null; Ball.shotBy = null
        Match.possession = lado
        _emJogo()

        // tres companheiros lancados, abertos em leque
        teams[lado].filter(p => !p.gk && p !== portador)
          .sort((a, b) => (b.pos.x - a.pos.x) * d).slice(0, 3)
          .forEach((p, i) => {
            p.pos.set(clamp(bx + d * rnd(11, 24), -HALF_L + 4, HALF_L - 4), 0,
                      clamp(bz + (i - 1) * rnd(9, 15), -HALF_W + 3, HALF_W - 3))
            p.prev.copy(p.pos); p.vel.set(0, 0, 0)
          })

        // o adversario estava atacando: so os dois primeiros voltam a tempo.
        // O gol deles fica em +dir, porque `dir` e a direcao de ataque de `lado`.
        teams[adv].filter(p => !p.gk).forEach((p, i) => {
          const x = i < 2 ? d * rnd(28, 40) : bx - d * rnd(2, 20)
          p.pos.set(clamp(x, -HALF_L + 3, HALF_L - 3), 0,
                    clamp(p.pos.z + rnd(-5, 5), -HALF_W + 2, HALF_W - 2))
          p.prev.copy(p.pos); p.vel.set(0, 0, 0)
        })

        Director.mode = 1; Director.free = false          // TRANSMISSAO: campo aberto
        $('cam').textContent = Director.names[1]
        Toast.show('CONTRA-ATAQUE', CFG.teams[lado].tag, 1400)
        Events.add('Contra-ataque de ' + CFG.teams[lado].tag)
        return true
      }

      // ATAQUE / PRESSAO — o time inteiro sobe e o adversario fecha o bloco
      // baixo. Usado para "ataque do time adversario": e o mesmo evento, com
      // `lado` apontando para o outro time.
      case "attack":
      case "pressure": {
        const d = CFG.teams[lado].dir
        const bx = d * (HALF_L - rnd(20, 34)), bz = rnd(-HALF_W * .55, HALF_W * .55)
        const portador = _maisPertoDe(lado, bx, bz)
        if(!portador) return false
        ...
        // quem ataca ocupa o ultimo terco; ninguem fica atras da linha da bola
        teams[lado].filter(p => !p.gk && p !== portador).forEach((p, i) => {
          p.pos.set(clamp(bx + d * rnd(2, 20), -HALF_L + 5, HALF_L - 5), 0,
                    clamp(-HALF_W * .7 + (i / 9) * HALF_W * 1.4 + rnd(-4, 4),
                          -HALF_W + 2, HALF_W - 2))
          p.prev.copy(p.pos); p.vel.set(0, 0, 0)
        })
        // bloco baixo: duas linhas na frente da propria area
        teams[adv].filter(p => !p.gk).forEach((p, i) => {
          const linha = i < 5 ? 13 : 21
          p.pos.set(d * (HALF_L - linha + rnd(-1.5, 1.5)), 0,
                    clamp(-18 + (i % 5) * 9 + rnd(-2, 2), -HALF_W + 2, HALF_W - 2))
          p.prev.copy(p.pos); p.vel.set(0, 0, 0)
        })

        Director.mode = 1; Director.free = false
        $('cam').textContent = Director.names[1]
        Toast.show('PRESSÃO', CFG.teams[lado].tag + ' no ataque', 1300)
        return true
      }
      ...
      // Eventos sem encenacao propria — ou o lance ja esta acontecendo em
      // campo, ou o que muda e administrativo e nao tem imagem. Devolver false
      // deixa o chamador saber que nao houve mudanca de cena, em vez de fingir
      // que encenou.
      case "offside": {
        Audio2.whistle(false)
        Ball.vel.set(0,0,0); Ball.spin.set(0,0,0); Ball.owner=null
        _reacaoDoTime(lado, 'reclamar', 1.8, .45)
        ;(teams[adv]||[]).filter(p=>!p.gk).slice(0,4)
          .forEach(p=>{ p.pose='aponta'; p.poseT=1.8 })
        Toast.show('IMPEDIMENTO', CFG.teams[lado].tag, 1300)
        Match.phase='set'; Match.wait=1.2; Match.possession=adv
        return true
      }
      case "injury": {
        const lesionado=_algumDeLinha(lado)
        if(lesionado){ lesionado.pose='maos_cabeca'; lesionado.poseT=3.2 }
        Match.phase='set'; Match.wait=1.8
        return !!lesionado
      }
      case "var":
        Audio2.whistle(false); Toast.show('VAR','checagem em andamento',1800)
        Match.phase='set'; Match.wait=1.8; return true
      case "sub":
        return false
