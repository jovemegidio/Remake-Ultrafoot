...
function PhaseCard({title,color,children}:{title:string;color:string;children:React.ReactNode}) { return <div className="rounded-xl border border-white/[0.08] bg-[var(--uf-bg-surface)] p-5"><h2 className={cn("mb-5 flex items-center gap-2 text-lg font-black",color)}><ShieldCheck className="h-5 w-5"/>{title}</h2><div className="space-y-4">{children}</div></div> }
...
function Metric({label,value}:{label:string;value:string|number}){return <div className="rounded-xl border border-white/[0.08] bg-[var(--uf-bg-surface)] p-5"><p className="text-xs uppercase text-white/40">{label}</p><p className="mt-2 text-3xl font-black">{value}</p></div>}
