...
export default function PerformancePage() {
  ...
  return <div className="flex h-screen flex-col overflow-hidden bg-[#050508] text-white">
    <GameHeader team={team} />
    <main className="flex min-h-0 flex-1 flex-col px-6 pb-8 pt-5">
      <div className="flex items-end justify-between border-b border-white/[0.07]"><div><p className="text-xs font-semibold uppercase tracking-[.22em] text-[var(--brand)]">Performance Center 286</p><h1 className="mt-1 text-2xl font-black">Decisões de futebol integradas</h1></div><span className="mb-3 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-white/50">Projeções: {projectionEngine === "rust" ? "motor Rust nativo" : "fallback TypeScript"}</span></div>
...
function PhaseCard({title,color,children}:{title:string;color:string;children:React.ReactNode}) { return <div className="rounded-xl border border-white/[0.08] bg-[#0c0c10] p-5"><h2 className={cn("mb-5 flex items-center gap-2 text-lg font-black",color)}><ShieldCheck className="h-5 w-5"/>{title}</h2><div className="space-y-4">{children}</div></div> }
...
function Metric({label,value}:{label:string;value:string|number}){return <div className="rounded-xl border border-white/[0.08] bg-[#0c0c10] p-5"><p className="text-xs uppercase text-white/40">{label}</p><p className="mt-2 text-3xl font-black">{value}</p></div>}
