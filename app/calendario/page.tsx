...
function generateMonthFixtures(month: number, season: number, userTeam: Team) {
  ...
  return Array.from({ length: daysInMonth }, (_, idx) => {
    const day = idx + 1
    const fi = fixtureIndices.indexOf(day)
    if (fi === -1) return { day }
    const opponent = teams[fi % teams.length]
    return {
      day,
      fixture: {
        opponent,
        isHome: fi % 2 === 0,
        competition: fi % 3 === 0 ? "Copa do Brasil" : "Brasileirao",
        competitionShort: fi % 3 === 0 ? "CDB" : "BRA",
        time: fi % 2 === 0 ? "16:00" : "21:00",
      },
    }
  })
}
...
export default function CalendarioPage() {
  ...
  const fixtures = useMemo(
    () => generateMonthFixtures(currentMonth, season, userTeam),
    [currentMonth, season, userTeam]
  )
  const selectedFixture = selectedDay ? fixtures.find(f => f.day === selectedDay)?.fixture : null

  const firstDayOfMonth = new Date(season, currentMonth, 1).getDay()
      ...
      <div className="flex h-[calc(100vh-48px-90px)]">
        ...
        <aside className="w-80 flex-shrink-0 border-r border-white/5 bg-[#0d0d0d] p-5 flex flex-col">
          <div className="mb-4">
            <div className="text-[10px] text-white/40 uppercase tracking-wider font-medium">
              {weekDays[new Date(season, currentMonth, selectedDay || 1).getDay() === 0 ? 6 : new Date(season, currentMonth, selectedDay || 1).getDay() - 1]}
            </div>
            <div className="text-xl font-semibold text-white">
              {months[currentMonth]} {selectedDay || 1}, {season}
            </div>
          </div>
              ...
              {nextFixtures.map((f, i) => (
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    f.fixture!.isHome ? "bg-[#1db954]/20 text-[#1db954]" : "bg-white/10 text-white/60"
                  )}>
        ...
        <main className="flex-1 p-6 overflow-auto">
          <div className="flex items-center gap-2 mb-6">
            <button onClick={() => setCurrentMonth(m => (m - 1 + 12) % 12)} className="p-2 rounded-lg hover:bg-white/5 text-white/60 hover:text-white transition-colors">
              ...
              {monthsShort.map((m, i) => (
                <button
                  ...
                  className={cn(
                    ...
                    i === currentMonth ? "bg-white/10 text-white" : "text-white/40 hover:text-white/70"
                  )}
                >
                  {m}
                </button>
            ...
            <button onClick={() => setCurrentMonth(m => (m + 1) % 12)} className="p-2 rounded-lg hover:bg-white/5 text-white/60 hover:text-white transition-colors">
          ...
          <div className="bg-[#141414]/50 rounded-xl border border-white/5 overflow-hidden">
            <div className="grid grid-cols-7 border-b border-white/5">
              {weekDays.map(day => (
                <div key={day} className="p-3 text-center text-xs font-medium text-white/40 uppercase tracking-wider">{day}</div>
              ))}
            </div>
            <div className="grid grid-cols-7">
              {Array.from({ length: startOffset }).map((_, i) => (
                <div key={`e-${i}`} className="aspect-square p-2 border-r border-b border-white/5 bg-black/20" />
              ))}
              {fixtures.map(({ day, fixture }) => {
                ...
                return (
                  ...
                  >
                    <span className={cn("text-lg font-medium mb-1", isSelected ? "text-white" : "text-white/60")}>
                      {day}
                    </span>
                    {fixture && (
                      <div className={cn(
        ...
        <aside className="w-80 flex-shrink-0 border-l border-white/5 bg-[#0d0d0d] p-5 hidden xl:flex flex-col">
          ...
          <div className="space-y-4">
            <div className="rounded-lg overflow-hidden bg-[#1a1a1a] border border-white/5">
              <div className="relative aspect-video bg-gradient-to-br from-[#0a5c2b] to-[#0d7a3a] flex items-center justify-center">
                <Image src="/logos/brasileirao.png" alt="Brasileirao" width={64} height={64} className="object-contain drop-shadow-lg" />
              </div>
              <div className="p-3">
                <div className="text-xs text-[#1db954] mb-1">COMPETICAO</div>
                <div className="text-sm font-medium text-white line-clamp-2">Brasileirao Serie A — Tabela atualizada</div>
              </div>
            </div>
