...
#
# Uso:  powershell -File scripts\fast-update.ps1
#       (rode a partir da RAIZ do projeto no G:)
#
# Quando gerar o INSTALADOR completo (release): use `npm run tauri:build` em
# C:\ultrafoot-build com ambiente limpo (ver memoria: NAO rodar builds sobrepostas).
# ─────────────────────────────────────────────────────────────────────────────
$ErrorActionPreference = "Stop"
$G = (Resolve-Path "$PSScriptRoot\..").Path
$C = "C:\ultrafoot-build"

Write-Host "==> Sincronizando source G: -> C: (sem node_modules/target/.next)..." -ForegroundColor Cyan
...
# imagens/assets novos (public/images e public/brand — leves; NAO mexe em music/escudos)
...
Write-Host "==> Limpando processos antes de UMA build..." -ForegroundColor Cyan
...
Write-Host "==> tauri build --no-bundle (pula o makensis)..." -ForegroundColor Cyan
$env:Path = "$env:USERPROFILE\.cargo\bin;$env:Path"
Push-Location $C
cmd /c "npm run tauri:build -- --no-bundle"
$code = $LASTEXITCODE
Pop-Location
if ($code -ne 0) { Write-Host "BUILD FALHOU (exit $code) — veja o log acima." -ForegroundColor Red; exit $code }

Write-Host "==> Trocando o exe no jogo instalado..." -ForegroundColor Cyan
...
Write-Host "OK — jogo instalado atualizado (v$v). Abra o Ultrafoot para ver as mudancas." -ForegroundColor Green
