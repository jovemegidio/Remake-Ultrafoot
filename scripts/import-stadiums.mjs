...
const source = path.resolve(process.argv[2] ?? path.join(root, "Nova pasta", "estadios"))
...
for (const entry of files) {
  ...
  manifest[key] = `/stadiums/${targetName}`
}
...
console.log(`Imported ${Object.keys(manifest).length} stadium photos into ${output}`)
