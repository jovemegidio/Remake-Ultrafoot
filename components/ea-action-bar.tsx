...
export function EaActionBarProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ActionBarContext.Provider value={{ actions: actions ?? DEFAULT_ACTIONS, setActions }}>
      {children}
...
const HIDDEN_PATHS = ["/splash", "/novo-jogo"]
...
export function EaActionBar() {
  ...
  // Em partida ao vivo a propria tela controla os rodapes
  if (pathname.startsWith("/partida/ao-vivo")) return null
