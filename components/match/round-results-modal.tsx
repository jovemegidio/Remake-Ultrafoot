      ...
      <div className="relative w-full max-w-5xl max-h-[92vh] rounded-2xl bg-[#091012] border border-white/10 shadow-[0_30px_100px_rgba(0,0,0,.7)] flex flex-col overflow-hidden">
        ...
        <div className="shrink-0 px-5 sm:px-7 py-5 border-b border-white/[0.07] flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex h-11 w-11 items-center justify-center rounded-xl border border-[var(--brand)]/20 bg-[var(--brand)]/10">
          ...
          <button
            onClick={onContinue}
            className="flex items-center gap-2 rounded-xl bg-[var(--brand)] px-4 sm:px-6 py-2.5 text-sm font-black text-[var(--brand-ink)] transition hover:brightness-110"
          >
        ...
        <div className="grid shrink-0 grid-cols-2 sm:grid-cols-5 border-b border-white/[0.06] bg-black/20">
          {[
            ...
            <div key={label} className="flex items-center justify-center gap-3 border-r border-white/[0.05] px-3 py-3.5 last:border-r-0">
              <Icon className="hidden h-4 w-4 text-[var(--brand)]/65 md:block" />
            ...
            grouped.map(([competition, matches]) => (
              <div key={competition}>
                <div className="mb-2.5 flex items-center justify-between border-b border-white/[0.06] pb-2">
                  <h3 className="text-xs font-black uppercase tracking-[.16em] text-[var(--brand)]/85">{competition}</h3>
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          mine ? "border-[var(--brand)]/35 bg-[var(--brand)]/10 shadow-[inset_3px_0_0_var(--brand)]" : "border-white/[0.05] bg-white/[0.025]"
                        )}
                      >
                        {mine && <span className="absolute right-2 top-1 rounded-full bg-[var(--brand)] px-2 py-0.5 text-[8px] font-black uppercase tracking-wider text-[var(--brand-ink)]">Seu clube</span>}
