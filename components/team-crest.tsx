 ...
 * Uses plain <img> to bypass next/image quirks with raw github URLs.
 */
export function TeamCrest({
  ...
  const FallbackShield = () => (
    ...
    >
      ...
      >
        <span className="font-display-italic" style={{ color: resolvedTeam?.cor1 || "oklch(0.78 0.18 195)" }}>
  ...
  return (
    <div
      className={cn("relative flex shrink-0 items-center justify-center", container, className)}
      aria-label={`Escudo ${resolvedTeam?.nome || "Time"}`}
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
        src={escudoUrl || "/placeholder.svg"}
