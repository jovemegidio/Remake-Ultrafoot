...
function getRatingColor(rating: number): string {
  ...
  if (rating >= 80) return "from-[#1db954] to-[#2ecc71]"
...
function getStaminaColor(stamina: number): string {
  if (stamina >= 70) return "bg-[#1db954]"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md">
      <div className="w-full max-w-5xl bg-[#0d0d0d] rounded-2xl border border-white/10 shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-gradient-to-r from-[#1db954]/10 to-transparent">
        ...
        <div className="flex items-center justify-center gap-4 py-4 border-b border-white/5">
          <div className={cn(
            ...
            step === "select-out" ? "bg-[#1db954]/20 text-[#1db954]" : "text-white/40"
          )}>
            <div className={cn(
              "w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold",
              step === "select-out" ? "bg-[#1db954] text-black" : selectedOut ? "bg-[#1db954] text-black" : "bg-white/10"
            )}>
              {selectedOut ? "✓" : "1"}
            </div>
          ...
          <div className={cn(
            "flex items-center gap-2 px-4 py-2 rounded-lg transition",
            step === "select-in" ? "bg-[#1db954]/20 text-[#1db954]" : "text-white/40"
          )}>
            <div className={cn(
              "w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold",
              step === "select-in" ? "bg-[#1db954] text-black" : selectedIn ? "bg-[#1db954] text-black" : "bg-white/10"
            )}>
              {selectedIn ? "✓" : "2"}
            </div>
          ...
          <div className={cn(
            "flex items-center gap-2 px-4 py-2 rounded-lg transition",
            step === "confirm" ? "bg-[#1db954]/20 text-[#1db954]" : "text-white/40"
          )}>
            <div className={cn(
              "w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold",
              step === "confirm" ? "bg-[#1db954] text-black" : "bg-white/10"
            )}>
              3
            </div>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      player.redCard
                        ...
                        : "border-white/10 bg-white/5 hover:bg-white/10 hover:border-[#1db954]/50"
                    )}
                  >
                    <div
            ...
            <div className="space-y-4">
              <div className="text-center mb-6">
                <div className="text-sm text-white/60">
                  <span className="text-[#1db954] font-semibold">{selectedOut?.name}</span> sai. Selecione quem entra.
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {bench.map(player => (
                  <button
                    ...
                    className="flex items-center gap-3 p-3 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 hover:border-[#1db954]/50 transition-all text-left"
                  >
                    <div
                      className="h-12 w-12 rounded-lg flex items-center justify-center text-lg font-bold shadow-lg"
                      style={{ backgroundColor: team.cor1, color: team.cor2 }}
          ...
          {step === "confirm" && selectedOut && selectedIn && (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-center gap-8">
                ...
                {/* Arrow */}
                <div className="flex flex-col items-center">
                  <ArrowLeftRight className="h-10 w-10 text-[#1db954]" />
                  <div className="mt-2 text-[10px] text-white/40">{currentMinute}&apos;</div>
                </div>

                {/* Player In */}
                <div className="flex flex-col items-center">
                  <div className="text-[10px] text-[#1db954] uppercase tracking-wider mb-2 font-semibold">Entra</div>
                  <div
                    className="h-20 w-20 rounded-xl flex items-center justify-center text-3xl font-black shadow-lg border-2 border-[#1db954]/50"
                    style={{ backgroundColor: team.cor1, color: team.cor2 }}
                  >
                    {selectedIn.number}
                  </div>
              ...
              <div className="flex justify-center gap-4 mt-8">
                ...
                <Button
                  ...
                  className="px-8 bg-[#1db954] text-black hover:bg-[#1ed760] font-bold"
                >
