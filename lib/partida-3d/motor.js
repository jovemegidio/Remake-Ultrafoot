...
class Player{
  constructor(team,idx){
    ...
    this.def=FORMATION[idx]; this.role=this.def.r; this.gk=this.role==='GOL';
...
function gkBrain(p,dt){
  ...
  if(aerial&&p.pose!=='dive'){
    ...
    if(dist(p.pos,b.pos)<2.05&&b.cool<=0){
      ...
      b.offside=null; Stats.onTarget(b.shotBy);
  ...
  if(!b.owner&&b.cool<=0&&dist(p.pos,b.pos)<grab&&b.pos.y<2.45){
    ...
    if(speed>19&&Math.random()<.58){        // chute forte: espalma para fora
      ...
      if(speed>11){ Stats.onTarget(b.shotBy); Toast.show('DEFENDEU','',1100); Audio2.cheer(.5); }
    }
  }
  if(Ball.owner===p){
    p.think-=dt;
  ...
  for(const p of players){
    ...
    if(chasers.has(p)){
      ...
      target=new THREE.Vector3(
        ...
        clamp(s.z+(b.pos.z-s.z)*.32,-HALF_W+2,HALF_W-2));
